...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('deparments_csv', type=str, help='Path to the DEPARTMENT CSV file')

    def handle(self, *args, **options):
        csv_file = options['deparments_csv']
        count = 0

        df_deparments = pd.read_csv(csv_file)

        for index, row in df_deparments.iterrows():
            try:
